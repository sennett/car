require(['physicsjs'], function(Physics) {
	...
	Physics(function (world) {
		...
		var viewportBounds = Physics.aabb(0, 0, window.innerWidth, window.innerHeight), width = window.innerWidth, height = window.innerHeight, renderer
			;
		...
		var zero = Physics.body('compound', {
			x: width / 2 - S(80), y: height / 2, treatment: 'static', styles: {
				fillStyle: colors.red, lineWidth: 1, strokeStyle: colors.darkRed, alpha: 0.0001 //pixi bug
			}, children: [
				// coords of children are relative to the compound center of mass
				Physics.body('rectangle', {
					x: S(-50), y: 0, width: S(20), height: S(97), mass: 20
				}), Physics.body('rectangle', {
					x: S(50), y: 0, width: S(20), height: S(97), mass: 20
				}), Physics.body('rectangle', {
					x: 0, y: S(75), width: S(48), height: S(20), mass: 20
				}), Physics.body('rectangle', {
					x: -S(35), y: S(60), angle: Math.PI / 4, width: S(50), height: S(20), mass: 20
				}), Physics.body('rectangle', {
					x: S(35), y: S(60), angle: -Math.PI / 4, width: S(50), height: S(20), mass: 20
				}), Physics.body('rectangle', {
					x: 0, y: S(-75), width: S(48), height: S(20), mass: 20
		...
		var point = Physics.body('circle', {
			x: width / 2 + S(6), y: height / 2 + S(76), radius: S(13), treatment: 'static', styles: {
				fillStyle: colors.red, lineWidth: 1, strokeStyle: colors.darkRed, alpha: 0.0001 //pixi bug
				, mass: 80
			}
		});
		...
		var seven = Physics.body('compound', {
			x: width / 2 + S(86), y: height / 2 - S(34), treatment: 'static', styles: {
				fillStyle: colors.red, lineWidth: 1, strokeStyle: colors.darkRed, alpha: 0.0001 //pixi bug
			}, children: [
				Physics.body('rectangle', {
					x: S(35), y: -S(80), width: S(114), height: S(24), mass: 40
				}), Physics.body('rectangle', {
					x: S(51.5), y: S(2.5), angle: -70 * Math.PI / 180, width: S(160), height: S(28), mass: 40
				})
			]
		});
		...
		while (l--) {
			circles.push(Physics.body('circle', {
				x: Math.random() * width, y: Math.random() * height, radius: S(10), mass: 1, restitution: 0.5, styles: {
					strokeStyle: colors.darkBlue, fillStyle: colors.blue, lineWidth: 1
				}
			}));
		}
		...
		function wrapY() {
			var b, i, l, h = height + 100, w = width / 2, x = w
				;
		...
		setTimeout(function () {
			...
			var edgeBounce = Physics.behavior('edge-collision-detection', {
				aabb: viewportBounds, restitution: 0, cof: 0.8
			});
		...
		// add behaviors
		world.add([
			Physics.behavior('constant-acceleration'), Physics.behavior('body-impulse-response'), Physics.behavior('body-collision-detection'), Physics.behavior('sweep-prune')
		]);
