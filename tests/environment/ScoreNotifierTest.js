define(['environment/ScoreNotifier', 'domain/Car', 'environment/renderer/FacadeRenderer'], function (ScoreNotifier, Car, FacadeRenderer) {
	...
	var assertGenerationAverageScoreProvided = function(){
		expect(this.onNewGenerationAverageScoreListener).toHaveBeenCalledWith('generation ID', 1.5);
	};
	...
	var assertCarCompleteListenerBound = function(){
		...
		expect(this.onCarSimulationCompleteListener).toHaveBeenCalled();
	};
	...
	var bindPublicEventListenersToScoreNotifier = function(){
		...
		this.onCarSimulationCompleteListener = jasmine.createSpy('on car simulation complete dummy');
		this.scoreNotifier.onCarSimulationComplete(this.onCarSimulationCompleteListener);
